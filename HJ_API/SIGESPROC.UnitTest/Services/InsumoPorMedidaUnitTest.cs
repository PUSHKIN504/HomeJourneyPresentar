...
{   [TestClass]
    public class InsumoPorMedidaUnitTest
    {
        private readonly InsumoService _insumoService;
        private readonly IMapper _mapper;
        public Mock<InsumoPorMedidaRepository> MockInsumoPorMedidaRepository { get; private set; }

        public InsumoPorMedidaUnitTest()
        {
            MockInsumoPorMedidaRepository = new Mock<InsumoPorMedidaRepository>();
        ...
        [TestMethod]
        public void InsumoPorMedidaCrear()
        {
            try
            {
                MockInsumoPorMedidaRepository.Setup(pl => pl.Insert(It.IsAny<tbInsumosPorMedidas>()))
                    .Returns(new RequestStatus { CodeStatus = 1, MessageStatus = "Exito" });

                var result = _insumoService.InsertarInsumoPorMedida(It.IsAny<tbInsumosPorMedidas>());
