...
{
    [TestClass]
    public class NotificacionUnitTest
    {
        ...
        public void NotificacionCreate()
        {
            ...
            MockNotificacionRepository.Setup(pr => pr.Insert(It.IsAny<tbNotificaciones>()))
              .Returns(new RequestStatus { CodeStatus = 1, MessageStatus = "Exito" });

            var result = _proyectoService.InsertarNotificacion(modelo);
        ...
        public void NotificacionUpdate()
        {
            ...
            MockNotificacionRepository.Setup(pr => pr.Update(It.IsAny<tbNotificaciones>()))
           .Returns(new RequestStatus { CodeStatus = 1, MessageStatus = "Exito" });

            var result = _proyectoService.ActualizarNotificacion(modelo);
