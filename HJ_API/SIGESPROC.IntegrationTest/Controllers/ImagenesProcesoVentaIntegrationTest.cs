...
{
    [TestClass]
    public class ImagenesProcesoVentaIntegrationTest : DbContextMocker
    {
        private const string ApiKey = "4b567cb1c6b24b51ab55248f8e66e5cc";

        [TestMethod]
        public async Task ImagenesProcesoVentaInsertar()
        {
            // Creamos el cliente
            ...
            var ImagenesProcesoVentaMock = ImagenesProcesoVentaMocks.CrearMockProcesoVenta();


            // Serializa el objeto a JSON para enviarlo en el cuerpo de la petición
            var contenido = new StringContent(
                System.Text.Json.JsonSerializer.Serialize(ImagenesProcesoVentaMock),
                System.Text.Encoding.UTF8,
                "application/json"
            );

            // Realiza la petición a la URL tipo POST
            var response = await cliente.PostAsync("https://localhost:44337/api/ImagenesProcesoVenta/Insertar", contenido);
        ...
        [TestMethod]
        public async Task ImagenesProcesoVentaEditar()
        {
            var cliente = factory.CreateClient();
            cliente.DefaultRequestHeaders.Add("XApiKey", ApiKey);

            var ImagenesProcesoVentaMock = ImagenesProcesoVentaMocks.EditarMockProcesoVenta();
            ...
            var response = await cliente.PutAsync("https://localhost:44337/api/ImagenesProcesoVenta/Actualizar", contenido);
