...
{
    [TestClass]
    public class RetrasoIntegrationTest : DbContextMocker
    {

         private const string ApiKey = "4b567cb1c6b24b51ab55248f8e66e5cc";

        [TestMethod]
        public async Task RetrasoActualizar()
        {
            ...
            var mock = RetrasoMock.ActualizarRetrasoMock();
            ...
            var response = await cliente.PutAsync("https://localhost:44337/api/Retraso/Actualizar", contenido);
        ...
        [TestMethod]
        public async Task RetrasoInsertar()
        {
            //creamos el cliente
            var cliente = factory.CreateClient();
            ...
            var mock = RetrasoMock.InsertarRetrasoMock();

            // Serializa el objeto a JSON para enviarlo en el cuerpo de la peticion
            var contenido = new StringContent(System.Text.Json.JsonSerializer.Serialize(mock), System.Text.Encoding.UTF8, "application/json");

            // Aquí hace la petición a la URL tipo Post
            var response = await cliente.PostAsync("https://localhost:44337/api/Retraso/Insertar", contenido);
